...
#define HELP_CACHE_DOCLIST 1 /* whether to cache the DocList between help_mbox_open calls */
...
{
  const struct Email *h1 = *(const struct Email **) a;
  const struct Email *h2 = *(const struct Email **) b;
...
static void *help_doc_clone(const void *item)
{
  ...
  dup->free_data = help_doc_meta_free;
  /* struct Body */
...
static struct Email *help_doc_from(const char *file)
{
  ...
  /* struct Email::data (custom metadata) */
  hdoc->data = meta;
  hdoc->free_data = help_doc_meta_free;
  /* struct Body */
...
static void help_dir_read(const char *path)
{
  ...
  HDType help_msg_top_type = ((HDMeta *) help_msg_top->data)->type;
...
static int help_mbox_open_append(struct Context *ctx, int flags)
{
...
static int help_msg_commit(struct Context *ctx, struct Message *msg)
{
  mutt_debug(1, "entering help_msg_commit\n");
  return -1;
}

static int help_msg_close(struct Context *ctx, struct Message *msg)
{
...
static int help_msg_padding_size(struct Context *ctx)
{
  mutt_debug(1, "entering help_msg_padding_size\n");
  return -1;
}

static int help_tags_edit(struct Context *ctx, const char *tags, char *buf, size_t buflen)
{
...
static int help_path_canon(char *buf, size_t buflen, const char *folder)
{
